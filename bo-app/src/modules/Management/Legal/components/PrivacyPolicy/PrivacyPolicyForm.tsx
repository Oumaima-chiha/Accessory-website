...
import type { IPrivacyPolicy, IReduxLegal } from '../../interfaces';
import {
  useAddPrivacyPolicyMutation,
  useUpdatePrivacyPolicyMutation,
} from '../../redux';

const PrivacyPolicyForm = (): React.ReactElement => {
  const { t } = useTranslation(['legal', 'common', 'fields']);
  const [addPrivacyPolicyHandler, { isLoading: isAddPrivacyLoading }] =
    useAddPrivacyPolicyMutation();
  const [updatePrivacyPolicyHandler, { isLoading: isUpdatePrivacyLoading }] =
    useUpdatePrivacyPolicyMutation();
  const location = useLocation();
  const navigate = useNavigate();
  const { state }: { state: FormProps<IPrivacyPolicy> } = location;
  const formik = useFormik({
    ...
    onSubmit: async values => {
      ...
      if (state?.type === FORM_TYPE.ADD) {
        await addPrivacyPolicyFn(values);
      } else {
        await updatePrivacyPolicyFn({
          id: state?.form?.id,
          ...values,
        });
      }
  ...
  const addPrivacyPolicyFn = async (
    payload: IReduxLegal.CreatePrivacyPolicyPayload,
  ): Promise<void> => {
    try {
      await addPrivacyPolicyHandler(payload).unwrap();
      displayToast(
        t('success_msgs.privacy_policy_success', { context: 'add' }),
        'success',
      );
  ...
  const updatePrivacyPolicyFn = async (
    payload: IReduxLegal.UpdatePrivacyPolicyPayload,
  ): Promise<void> => {
    try {
      await updatePrivacyPolicyHandler(payload).unwrap();
      displayToast(
        t('success_msgs.privacy_policy_success', { context: 'update' }),
        'success',
      );
      navigate(legalManagementRoute);
    } catch (err) {
  ...
  return (
    <Form
      ...
      title={`${t(`shared:${state?.type}`)} ${t('privacy_policy')}`}>
      <VStack gap={3} alignItems="start">
        ...
        {state?.type !== FORM_TYPE.VIEW && (
          <FormControlButtons
            isLoading={isAddPrivacyLoading || isUpdatePrivacyLoading}
          />
        )}
        {state?.type === FORM_TYPE.VIEW && (
          <ApprovalDetails
