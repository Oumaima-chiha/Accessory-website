  ...
  try {
    ...
    if (!existingRole) {
      const superAdminRole = await prisma.role.create({
        data: {
          name: 'superAdmin',
          // Add any other properties you want to set for the super admin role
        },
      });
      ...
      await prisma.backOfficeUser.create({
        data: {
          ...
          firstname: 'Super',
          lastname: 'Admin',
          password: await bcrypt.hash('test1234', 10), // Hash the password
          email: 'super.admin@falshdrop.com',
          active: true,
          roleId: superAdminRole.id, // Assign the super admin role ID
...
async function seedRoleCategories() {
  try {
    const roleCategories = [
      ...
      { "name": "SURVEY QUESTIONS MANAGEMENT" },
      { "name": "SURVEY QUESTIONS OPTION MANAGEMENT" },
      { "name": "CONTENT APPROVAL" },
      { "name": "FAQ CATEGORY MANAGEMENT" }
    ]


    for (const category of roleCategories) {
      await prisma.roleCategory.create({ data: category });
    }

    console.log('Role categories seeded successfully.');
  } catch (error) {
    console.error('Error seeding role categories:', error);
  }
}
...
async function seedRolePermissions() {
  ...
  try {
    const rolePermissions =require('./RolesOnPermission.json')

    for (const rolePermission of rolePermissions) {
      await prisma.rolesOnPermission.create({
        data: {
