...
module.exports = {
  ...
  getJewelryByTagId: async (req, res) => {
    try {
      ...
      const jewelries = jewelryWithTags.map(
        (jewelryWithTag) => jewelryWithTag.jewelry,
      );
  ...
  addToFavorites: async (req, res) => {
    try {
      ...
      if (existingItem) {
        return res
          .status(400)
          .json({ message: "Jewelry already in favorites" });
      }
      ...
      res
        .status(200)
        .json({ message: "Jewelry added to favorites successfully" });
    } catch (error) {
  ...
  removeFromFavorites: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({
          message: "Jewelry removed from favorites successfully",
          removedId: existingItem.id,
        });
    } catch (error) {
  ...
  removeAllFavorites: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "All items removed from favorites successfully" });
    } catch (error) {
